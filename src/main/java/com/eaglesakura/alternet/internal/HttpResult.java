...
import com.eaglesakura.alternet.NetworkProfile;
...
import com.eaglesakura.alternet.error.HttpAccessRetryFailedException;
...
public abstract class HttpResult<T> extends Result<T> {
    ...
    protected final Alternet mConnector;
    ...
    public HttpResult(Alternet connector, ConnectRequest request, RequestParser<T> parser) {
        this.mParser = parser;
    ...
    protected T parseFromStream(CallbackHolder<T> callback, HttpHeader respHeader, InputStream stream, ICacheWriter cacheWriter, MessageDigest digest) throws Exception {
        ...
        StreamProxy controller = mConnector.getStreamProxy();
        ...
        try {
            ...
            if (controller != null) {
                readStream = controller.wrap(this, respHeader, parseStream);
            } else {
                readStream = stream;
            }
    ...
    protected ICacheWriter newCacheWriter(HttpHeader header) {
        try {
            CacheController controller = mConnector.getCacheController();
            if (controller != null) {
